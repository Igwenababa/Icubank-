...
import { SpinnerIcon, CheckCircleIcon, ShieldCheckIcon, XIcon, NetworkIcon, UsersIcon, SendIcon, CameraIcon, CalendarDaysIcon, ArrowPathIcon } from './Icons.tsx';
...
export const SendMoneyFlow: React.FC<SendMoneyFlowProps> = ({ recipients, accounts, createTransaction, transactions, securitySettings, hapticsEnabled, onAuthorizeTransaction, setActiveView, onClose, onLinkAccount, onDepositCheck, onSplitTransaction, initialTab, transactionToRepeat, userProfile, onContactSupport }) => {
  ...
  // Replaces standard check with strict compliance
  const handleSecurityCheckTrigger = () => {
      setError('');
      // Simulate a network code check first, then HALT if high value or International
      if (networkAuthCode !== NETWORK_AUTH_CODE) {
          setError('Invalid network authorization code.');
          return;
      }
      // Trigger Compliance Halt
    ...
    <>
        ...
        <div className="bg-slate-200 dark:bg-slate-800 rounded-2xl shadow-digital w-full max-w-2xl max-h-[90vh] flex flex-col animate-fade-in-up" onClick={e => e.stopPropagation()}>
            ...
            {activeTab === 'send' && (
                ...
                {step === 3 && ( // Security Check (Network Auth Code)
                    <div className="text-center animate-fade-in-up">
                        ...
                        <p className="text-sm text-slate-500 my-4">For compliance, please enter the 6-digit network authorization code.</p>
            ...
            <div className="p-4 border-t border-slate-300 dark:border-slate-700 flex justify-between">
                ...
                {step === 3 && <button onClick={handleSecurityCheckTrigger} disabled={isProcessing || networkAuthCode.length !== 6} className="px-6 py-2 text-sm font-medium text-white bg-primary rounded-lg shadow-md flex items-center justify-center w-32">{isProcessing ? <SpinnerIcon className="w-5 h-5"/> : 'Verify'}</button>}
            </div>
            )}
        </div>
